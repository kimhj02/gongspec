...
import type { HolidayMap } from '@/lib/korean-holidays'
                ...
                {week.map((day, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {holiday ? <span className="calendar-holiday">{holiday}</span> : null}
                    </button>
                  )
                })}
              </div>
              {laneCount || overflowRows ? (
                <div
                  className="calendar-week-events"
                  style={{ gridTemplateRows: `repeat(${laneCount + overflowRows}, 18px)` }}
                >
                  {segments.map((segment) => (
                    <button
                      type="button"
                      key={`${weekIndex}-${segment.event.id}`}
                      className={`event-bar ${eventTypeClass(segment.event.type)} ${segment.continuesLeft ? 'is-continue-left' : ''} ${segment.continuesRight ? 'is-continue-right' : ''}`}
