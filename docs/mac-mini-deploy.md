...
- `~/actions-runner-gongspec/.env`만 복사 (파일은 커밋하지 않음. Desktop 경로는 쓰지 않음)
...
`.env`는 `~/actions-runner-gongspec/.env`에 둡니다. `~/Desktop/gongspec`는 iCloud/git 잠금으로 읽기가 멈출 수 있어 배포 경로로 쓰지 않습니다.
